...
There were four of these total, each worth 5 points (2 pts for initial submission; 3 for self-assessment).
...
### Overall integrative lab grades

```{r}
ggplot(ilabs_grades, aes(x = ilab_prop)) +
  geom_histogram(colour = "white", binwidth = 3, fill = wes_palette("Royal2")[5]) +
  labs(x = "") +
  theme_minimal() 
```
...
combined <- lab_grades %>% 
  ...
  left_join(dc_grades) %>% 
  select(fake_name, lab_grades, ilab_prop, dc_grade) %>% 

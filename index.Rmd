...
lab_grades <- labs %>% 
  ...
  mutate(lab_grade = case_when(
    # pluses
    adj_plus == floor(.2*total_labs) & 
      adj_check == total_labs ~ 92,
    adj_plus == floor(.4*total_labs) & 
      adj_check == total_labs ~ 96,
    adj_plus > floor(.4*total_labs) & 
      adj_check == total_labs ~ 100,
    # solid = A
    adj_check == total_labs ~ 90,
    between(adj_check, floor(.8*total_labs), ceiling(.8*total_labs)) ~ 80 + adj_bounce,
    between(adj_check, floor(.7*total_labs), ceiling(.7*total_labs)) ~ 75 + adj_bounce,
    between(adj_check, floor(.6*total_labs), ceiling(.6*total_labs)) ~ 70 + adj_bounce,
    adj_check < floor(.6*total_labs) ~ 65 + adj_bounce,
    # if minuses remain
